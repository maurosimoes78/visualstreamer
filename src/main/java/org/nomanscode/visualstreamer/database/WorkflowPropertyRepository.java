...
@Repository
public class WorkflowPropertyRepository extends PropertyRepository {
    ...
    {
        try (PreparedStatement stmt = conn.prepareStatement("SELECT workflow_id, key, value FROM workflows.property ORDER BY workflow_id, key")) {
    ...
    {
        String sql = "INSERT INTO workflows.property (id, workflow_id, key, value) VALUES (?, ?, ?, ?) ON CONFLICT (workflow_id, key) DO UPDATE SET value = ?";
    ...
    protected boolean del(Connection conn, Property.Id id) throws SQLException
    {
        ...
        try (PreparedStatement stmt = conn.prepareStatement("DELETE FROM workflows.property WHERE workflow_id = ? AND key = ?")) {
